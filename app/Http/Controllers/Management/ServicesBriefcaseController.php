...
class ServicesBriefcaseController extends Controller
{
    ...
    /**
     * Get procedure by chRecordId.
     *
     * @param  int  $briefcaseId
     * @return JsonResponse
     */
    public function getByChRecordId(Request $request, int $chRecordId): JsonResponse
    {
        $id = ChRecord::where('ch_record.id',$chRecordId)
        ->select('admissions.briefcase_id AS briefcase_id')
        ...
        $ServicesBriefcase = ServicesBriefcase::select(
            ...
            ->leftjoin('product', 'manual_price.product_id', 'product.id')
            ->leftjoin('product_supplies', 'manual_price.supplies_id', 'product_supplies.id')
            ->where('briefcase_id', $id[0]['briefcase_id']);
        if ($request->type == 1) {
        } else if ($request->type == 2) {
